...
class CourseSearchTool(Tool):
    ...
    def __init__(self, vector_store: VectorStore):
        ...
        self.last_sources: List[Dict[str, Optional[str]]] = (
            []
        )  # Track sources from last search
...
class CourseOutlineTool(Tool):
    ...
    def execute(self, course_name: str) -> str:
        ...
        if not resolved_title:
            ...
            available = (
                ", ".join([c.get("title", "Unknown") for c in all_metadata])
                if all_metadata
                else "none"
            )
        ...
        all_metadata = self.store.get_all_courses_metadata()
        course_data = next(
            (c for c in all_metadata if c["title"] == resolved_title), None
        )
